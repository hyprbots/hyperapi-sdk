...
def _fake_key(kind: str, body: str) -> str:
    """Build credential-shaped test data without embedding a scannable token."""
    return f"hk_{kind}_{body}"
...
def test_rate_limit_error_carries_window_seconds_and_degraded():
    """Window and degradation metadata round-trip through the constructor."""
    err = RateLimitError(
...
def test_exception_message_redacts_api_keys():
    ...
    fake_key = _fake_key("live", "abc123def456ghi789jkl0")
    err = ParseError(f"Invalid token: {fake_key}")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            f"Bad key {_fake_key('test', 'abcdefgh1234')}",
            [_fake_key("test", "abcdefgh1234")],
            ["hk_***redacted"],
        ),
...
def test_parse_retry_after_http_date_form():
    """A gateway may emit the RFC-7231 HTTP-date form of Retry-After."""
...
def test_client_repr_masks_api_key():
    ...
    client = HyperAPIClient(api_key=_fake_key("live", "abcdef0123456789xyz"))
    text = repr(client)
